-- Clean up any duplicate or invalid reminders that might be causing issues
-- First, let's add a unique constraint to prevent duplicates based on user, type, title, and due date
DROP INDEX IF EXISTS unique_system_reminder;
...
-- Clean up any reminders with invalid UUIDs (they shouldn't exist, but just in case)
UPDATE reminders 
SET is_deleted = true 
