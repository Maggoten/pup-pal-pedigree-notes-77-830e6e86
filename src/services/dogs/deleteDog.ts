...
export async function deleteDog(id: string, mode: DeletionMode = 'soft'): Promise<boolean> {
  if (!id) {
  ...
  try {
    ...
    console.log(`Starting dog ${mode} deletion process for:`, id);

    if (mode === 'soft') {
      // Perform soft deletion by setting the deleted_at timestamp
      ...
      if (error) {
        console.error('Error soft deleting dog:', error.message);
        throw new Error(error.message);
      }
      
      console.log('Successfully soft deleted dog:', id);
      return true;
    } else {
      // Begin a series of cleanup operations for hard deletion
      ...
      console.log('Hard deleting dog:', id);
      ...
      if (response.error) {
        console.error('Error hard deleting dog:', response.error.message);
      ...
      console.log('Successfully hard deleted dog:', id);
      return true;
    }
  } catch (error) {
...
export async function checkDogDependencies(dogId: string) {
  if (!dogId) {
    throw new Error('Dog ID is required');
  }

  try {
    const { data, error } = await supabase.rpc('check_dog_dependencies', { dog_id: dogId });
