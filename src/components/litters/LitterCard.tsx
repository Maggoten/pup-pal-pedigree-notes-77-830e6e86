...
const LitterCard: React.FC<LitterCardProps> = ({
  ...
  return <Card className={`h-full overflow-hidden transition-all duration-300 ${isSelected ? 'ring-2 ring-primary shadow-lg transform scale-[1.02]' : 'hover:scale-[1.01]'} cursor-pointer bg-white border border-warmbeige-100 hover:shadow-md`} onClick={() => onSelect(litter)}>
      <CardHeader className="pb-3 pt-4 px-4">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3 flex-1">
            <Avatar className="h-12 w-12 border-2 border-white shadow-sm">
              {damImageUrl ? <AvatarImage src={damImageUrl} alt={litter.damName} className="object-cover" /> : <AvatarFallback className="bg-warmgreen-100 text-warmgreen-800 font-medium">
          ...
          <div className="flex flex-col gap-2 items-end">
            {/* Status Badge */}
            <Badge variant={litter.archived ? "secondary" : "default"} className={`text-xs px-2.5 py-0.5 rounded-full ${litter.archived ? 'bg-gray-100 text-gray-600 hover:bg-gray-200' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
      ...
      <CardContent className="pb-4 pt-0 px-4">
        <div className="space-y-3 text-sm">
          <div className="flex items-center gap-1.5">
            ...
            <span>{t('litter.labels.bornLabel', {
              date: format(birthDate, 'MMM d, yyyy')
            })}</span>
          </div>
          
          {puppyCount > 0 && <div className="flex items-center gap-1.5">
              <Users className="h-3.5 w-3.5 text-primary opacity-70" />
              <div className="flex gap-2 flex-wrap">
                <span>{t('litter.labels.totalCount', {
                count: aliveCount
              })}</span>
                <span className="text-blue-500 font-medium">{maleCount} ♂</span>
                <span className="text-pink-500 font-medium">{femaleCount} ♀</span>
                {deadCount > 0 && <span className="text-muted-foreground font-medium">
                    † {deadCount}
                  </span>}
              </div>
            </div>}
