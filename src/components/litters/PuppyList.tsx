...
const PuppyList: React.FC<PuppyListProps> = ({
  ...
  const getLatestMeasurement = (puppy: Puppy, type: 'weight' | 'height') => {
    ...
    const latestEntry = log.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
    const value = type === 'weight' ? (latestEntry as {
      weight: number;
    }).weight : (latestEntry as {
      height: number;
    }).height;
  ...
  return <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        ...
        <tbody>
          {puppies.map(puppy => <Dialog key={puppy.id}>
              <DialogTrigger asChild>
                <tr className={`border-b hover:bg-muted/50 cursor-pointer ${selectedPuppyId === puppy.id ? 'bg-primary/5' : ''}`} onClick={() => onPuppyClick && onPuppyClick(puppy)}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8">
                        {puppy.imageUrl ? <AvatarImage src={puppy.imageUrl} alt={puppy.name} className="object-cover" /> : <AvatarFallback className="bg-primary/10 text-primary text-xs">
                  ...
                  <td className="px-4 py-3">
                    ...
                    onAddMeasurement && onAddMeasurement(puppy);
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={e => handleDeletePuppy(puppy.id, puppy.name, e)}>
