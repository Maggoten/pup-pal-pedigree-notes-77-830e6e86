...
const PregnancyTimeline: React.FC<PregnancyTimelineProps> = ({ matingDate, expectedDueDate }) => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="relative ml-6 border-l-2 border-gray-200 pl-6 pb-6 space-y-8">
            ...
            return (
              <div key={index} className="relative">
                <div className={`absolute -left-9 p-1.5 rounded-full ${
                  isPast ? 'bg-green-500' : isCurrent ? 'bg-amber-500' : 'bg-gray-300'
                }`}>
                  {event.icon}
                </div>
                <div className={`${isCurrent ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-200'} border rounded-lg p-4`}>
                  ...
                  <div className="mt-2 text-sm">
                    ...
                    {isPast && <span className="ml-2 text-green-600 font-medium">Completed</span>}
                    {isCurrent && <span className="ml-2 text-amber-600 font-medium">Current Stage</span>}
                  </div>
                </div>
              </div>
            );
